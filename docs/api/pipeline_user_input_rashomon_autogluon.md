...
class arsa_ml.pipelines.pipelines_user_input.<strong>BuildRashomonAutogluon</strong>(predictor, test_data, df_name, base_metric, delta, feature_imp_needed, converter_results_directory)
</div>
</br>
This is a subclass of a <code>Builder</code> abstract class, which provides pipeline for creating and exploring the Rashomon Set from user-provided models from AutoGluon framework.
</br>
</br>
...
<div style="background-color: lightgray; padding: 3px; border-radius: 0px;">
    <strong>BuildRashomonAutogluon Pipeline – Key Steps</strong>
</div>
<br>
<ol>
    <li><strong>Initialization (<code>__init__</code>)</strong> – Convert AutoGluon <code>TabularPredictor</code> object and test data to format for Rashomon Set analysis.</li>
...
&emsp;Builds the Rashomon Set pipeline from AutoGluon output.<br>
